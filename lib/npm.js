  ...
  start: function start (cb) {
    ...
    this.hosts(function (host, done) {
      self._ssh(host, ["cd "+ self.deploymentOptions.currentPath(host), "npm install"], function() {
        var outFile = self.deploymentOptions.outputFile || self.deploymentOptions.name+".log" || "output.log";
        self._ssh(host, ["cd "+ self.deploymentOptions.currentPath(host)+ " ; nohup npm start > logs/"+ outFile + " 2>&1 &"], done);
      });
    }, cb);
  },
