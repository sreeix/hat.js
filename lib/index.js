...
module.exports = app = {
  ...
  exec: function exec(commands, options, cb){
    var stage = options.stage || app.deploymentOptions.defaultStage, deploymentTypePath, repositoryTypePath;
    app.options = options;
    ...
    if(app.deploymentOptions.via){
      _.extend(app, require(__dirname + path.sep + app.deploymentOptions.via))
    }
  ...
  deploy: function deploySSH (cb){
    app._serial([app.setup, app.checkout, app.changeSymlinks, app.restart], cb);
  },
