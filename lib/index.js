...
module.exports = foo = {
  exec: function exec(commands, options){
    ...
    var deploymentOptions = require(process.cwd() + "/deploy/" + stage);
      var deploymentTypePath = __dirname + path.sep + deploymentOptions.deploymentType
      if(fs.existsSync(deploymentTypePath)) _.extend(foo, require(deploymentTypePath));
...
// This is broken for multiple machines... as only one will be executed. Need to use async.parallel here and similar places.
  ...
  cleanup: function(opt, cb){
    _.each(opt.hosts, function(host){
      _ssh(host, [ "cd "+ opt.releasesPath(host), "count=`ls -t . | wc -l` && [ $count -gt 5 ] && ls -t . | head -n$(($count - 5)) | xargs rm -rf"], cb);
    });
  }
};
